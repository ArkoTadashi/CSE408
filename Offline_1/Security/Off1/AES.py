from BitVector import *
import time
from collections import deque
Sbox = (
    ,
)

InvSbox = (
    ,
)

Mixer = [
    [BitVector(hexstring="02"), BitVector(hexstring="03"), BitVector(hexstring="01"), BitVector(hexstring="01")],
    [BitVector(hexstring="01"), BitVector(hexstring="02"), BitVector(hexstring="03"), BitVector(hexstring="01")],
    [BitVector(hexstring="01"), BitVector(hexstring="01"), BitVector(hexstring="02"), BitVector(hexstring="03")],
    [BitVector(hexstring="03"), BitVector(hexstring="01"), BitVector(hexstring="01"), BitVector(hexstring="02")]
]

InvMixer = [
    [BitVector(hexstring="0E"), BitVector(hexstring="0B"), BitVector(hexstring="0D"), BitVector(hexstring="09")],
    [BitVector(hexstring="09"), BitVector(hexstring="0E"), BitVector(hexstring="0B"), BitVector(hexstring="0D")],
    [BitVector(hexstring="0D"), BitVector(hexstring="09"), BitVector(hexstring="0E"), BitVector(hexstring="0B")],
    [BitVector(hexstring="0B"), BitVector(hexstring="0D"), BitVector(hexstring="09"), BitVector(hexstring="0E")]
]

b = BitVector(hexstring="4E")
int_val = b.intValue()
s = Sbox[int_val]
s = BitVector(intVal=s, size=8)
print(s.get_bitvector_in_hex())

AES_modulus = BitVector(bitstring='100011011')

bv1 = BitVector(hexstring="02")
bv2 = BitVector(hexstring="63")
bv3 = bv1.gf_multiply_modular(bv2, AES_modulus, 8)
print(bv3)

def addRoundKey(matrix, hKey):
    rMatrix = [
    [BitVector(hexstring=hKey[i + j * 8: i + j * 8 + 2]) for j in range(4)]          #chaangeessss
    for i in range(0, 8, 2)
    ]  

    new_matrix = [
    [matrix[i][j] ^ rMatrix[i][j] for j in range(4)]
    for i in range(4)
    ]

    return new_matrix




def left_shift_line(lst, shift):
    shifted_lst = deque(lst)
    shifted_lst.rotate(-shift)
    return line(shifted_lst)

def left_shift_matrix(matrix):
    for i in range(0,len(matrix)):
        matrix[i] = left_shift_line(matrix[i],i)
    return matrix

def right_shift_line(lst, shift):
    shifted_lst = deque(lst)
    shifted_lst.rotate(shift)
    return line(shifted_lst)

def right_shift_matrix(matrix):
    for i in range(0,len(matrix)):
        matrix[i] = right_shift_line(matrix[i],i)
    return matrix



def byteSubstitutionLine(line):
    return [BitVector(intVal=Sbox[line[i].intValue()],size=8) for i in range(0,len(line))]

def byteSubstitutionLineInv(line):
    return [BitVector(intVal=InvSbox[line[i].intValue()],size=8) for i in range(0,len(line))]

def byteSubstitution(state_mat):
   state_mat = [
    [BitVector(intVal=Sbox[state_mat[i][j].intValue()], size=8) for j in range(4)]
    for i in range(4)
   ]
   return state_mat

def byteSubstitutionInverse(state_mat):
   state_mat = [
    [BitVector(intVal=InvSbox[state_mat[i][j].intValue()], size=8) for j in range(4)]
    for i in range(4)
   ]
   return state_mat



def lineXOR(line1,line2):
    return [line1[i]^line2[i] for i in range(0,len(line1))]

def matrixXOR(matrix1, matrix2):
    return [lineXOR(row1, row2) for row1, row2 in zip(matrix1, matrix2)]            #kechaalllsss



def makeMatrix(mat):
    mat = [mat[i:i+4] for i in range(0,len(mat),4)]
    mat = [[BitVector(hexstring=mat[i][j]) for i in range(0,len(mat))] for j in range(0,len(mat[0]))]
    mat = list(map(list, zip(*mat)))
    return mat

def transpose_matrix(matrix):
    return list(map(list, zip(*matrix)))

def convert_to_ascii(matrix):
    ascii_string = ""
    for i in range(0,len(matrix)):
        for j in range(0,len(matrix[i])):
            ascii_string += chr(int(matrix[i][j].get_bitvector_in_hex(),16))
    return ascii_string

def flatten_matrix(matrix):
    return ''.join([''.join([ele.get_bitvector_in_hex() for ele in sub]) for sub in matrix])

def g(w,rc):  
    w = left_shift_line(w,1)
    w = byteSubstitutionLine(w)
    w[0] = w[0] ^ rc
    return w

# AES key scheduling
def key_scheduling(key,AES_LEN):
    all_keys = []
    key_round_0 = makeMatrix(key)
    all_keys.append(key_round_0)
    # print("Round 0")
    # print_bitvector_matrix(key_round_0)
    COL = len(key_round_0)
    round_constant = BitVector(hexstring="01")
    for round in range(1,ROUND_COUNT[AES_LEN]+1):
        key_new_round = []
        key_new_round.append( xor_list(all_keys[round-1][0],func_g(all_keys[round-1][COL-1],round_constant)) )
        for i in range(1,len(all_keys[round-1])):
            key_new_round.append(xor_list(all_keys[round-1][i],key_new_round[i-1]))
        all_keys.append(key_new_round)
        # print("Round",round)
        # print_bitvector_matrix(key_new_round)
        round_constant = round_constant.gf_multiply_modular(BitVector(hexstring="02"),AES_MODULUS,8)

    # transpose all_keys
    for k in range(0,len(all_keys)):
        all_keys[k] = transpose_matrix(all_keys[k])
   
    return all_keys

def mix_columns(matrix,inv=False):
    mixer = Mixer
    if inv:
        mixer = InvMixer
    ret = []
    for _ in range(len(matrix)):
        ret.append([BitVector(intVal=0, size=8)] * len(matrix[0]))
    for i in range(0,len(matrix)):
        for j in range(0,len(matrix[0])):
            for k in range(0,len(matrix)):
                ret[i][j] = ret[i][j] ^ ( mixer[i][k].gf_multiply_modular( matrix[k][j],AES_MODULUS,8)   ) 
    
    return ret

def AES_ENCRYPT_ROUNDS(state,keys,AES_LEN):
    # print("Encrypt Round 0")
    # print_bitvector_matrix(state)

    print_bitvector_matrix(keys[0])

    state = xor_matrix(state,keys[0])
    print_round(0)
    print_bitvector_matrix(state)
    
    for round in range(1,ROUND_COUNT[AES_LEN]+1):
        print_round(round)
        # substitute bytes
        state = substitute_bytes_matrix(state)
        print_bitvector_matrix(state)
        # shift rows
        state = left_shift_matrix(state)
        print_bitvector_matrix(state)
        # mix columns
        if round != ROUND_COUNT[AES_LEN]:
            state = mix_columns(state)
            print_bitvector_matrix(state)
        # add round key
        state = xor_matrix(state,keys[round])
        print_bitvector_matrix(state)
       
    return state

def ENCRYPT_AES(AES_LEN,text,key):
    # key might not be of the required length, so resize it
    #key = resize_key(key,AES_LEN)
    # AES key scheduling
    key_schedule = key_scheduling(key,AES_LEN)

    while( len(text)%(AES_LEN//8) != 0 ):
        text += " "
    
    encrypted_text_list = ""
    encrypted_hex_list = ""

    start_time = time.time()

    for i in range(0,len(text),AES_LEN//8):
        split_text = text[i:i+AES_LEN//8]
        split_text = makeMatrix( convert_to_hex_string(split_text) )
        split_text = transpose_matrix(split_text)
        encrypted = AES_ENCRYPT_ROUNDS(split_text,key_schedule,AES_LEN)
        encrypted = transpose_matrix(encrypted)
        encrypted_text = convert_to_ascii( encrypted )
        encrypted_hex = flatten_matrix(encrypted)
        encrypted_text_list+=(encrypted_text)
        encrypted_hex_list+=(encrypted_hex)
   
    
    encryption_time = time.time() - start_time

    # print("Encrypted hex:",encrypted_hex_list)
    # print("Encrypted text:",encrypted_text_list)
    
    return encrypted_hex_list,encrypted_text_list,encryption_time

def process_text(plain_text,m):
    # Convert plain_text to hex
    hex_text = plain_text.encode("utf-8").hex()
    print("len :",len(hex_text))
    if len(hex_text) % m != 0:
        num_zeroes = m - (len(hex_text) % m)
        
        hex_text += '0' * int(num_zeroes) 

    return hex_text

def print_with_space(input_string):
    result = ' '.join([input_string[i:i+2] for i in range(0, len(input_string), 2)])
    print("In HEX:",result)


if __name__ == "__main__":
    k = 128
    # k = 192
    # k = 256
    print("Enter plain text:")
    #plain_text = input()
    plain_text="Never gonna give you up"
    print("Enter key:")
    #key = input()
    key = "BUET CSE19 Batch"
    #hText = plain_text.encode("utf-8").hex()
    hText = process_text(plain_text,k/4)
    hKey = process_text(key,k/4)

   
    print("\nKey:")
    print("In ASCII:", key)
    print_with_space(hKey)
    print("\nPlain Text:")
    print("In ASCII:", plain_text)
    print_with_space(hText)
    print()
    key_bin = format(int(hKey, 16), 'b')
    [encrypted_hex,encrypted_text,encryption_time] = ENCRYPT_AES(k,plain_text,key_bin)